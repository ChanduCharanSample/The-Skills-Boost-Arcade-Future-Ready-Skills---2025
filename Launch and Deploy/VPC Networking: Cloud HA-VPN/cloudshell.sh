...
# Ask once for the zone
read -p "Enter the zone to use (e.g. us-east1-c): " ZONE
REGION=$(echo "$ZONE" | rev | cut -d- -f2- | rev)

# Prompt once for VPN shared secret
read -p "Enter the VPN shared secret: " SHARED_SECRET
...
gcloud beta compute networks subnets create vpc-demo-subnet2 \
  --network vpc-demo --range 10.2.1.0/24 --region asia-east1
...
# VM instances
gcloud compute instances create vpc-demo-instance1 --zone us-east1-c --subnet vpc-demo-subnet1 --machine-type e2-medium
gcloud compute instances create vpc-demo-instance2 --zone asia-east1-b --subnet vpc-demo-subnet2 --machine-type e2-medium
...
gcloud compute instances create on-prem-instance1 --zone us-east1-b --subnet on-prem-subnet1 --machine-type e2-medium

# VPN Gateways
gcloud beta compute vpn-gateways create vpc-demo-vpn-gw1 --network vpc-demo --region us-east1
gcloud beta compute vpn-gateways create on-prem-vpn-gw1 --network on-prem --region us-east1

# Routers
gcloud compute routers create vpc-demo-router1 --region us-east1 --network vpc-demo --asn 65001
gcloud compute routers create on-prem-router1 --region us-east1 --network on-prem --asn 65002
...
gcloud beta compute vpn-tunnels create on-prem-tunnel0 \
  --peer-gcp-gateway vpc-demo-vpn-gw1 --region us-east1 --ike-version 2 \
...
# Interfaces & BGP peers
gcloud compute routers add-interface vpc-demo-router1 --interface-name if-tunnel0-to-on-prem \
  --ip-address 169.254.0.1 --mask-length 30 --vpn-tunnel vpc-demo-tunnel0 --region us-east1
gcloud compute routers add-bgp-peer vpc-demo-router1 --peer-name bgp-on-prem-tunnel0 \
  --interface if-tunnel0-to-on-prem --peer-ip-address 169.254.0.2 --peer-asn 65002 --region us-east1
gcloud compute routers add-interface vpc-demo-router1 --interface-name if-tunnel1-to-on-prem \
  --ip-address 169.254.1.1 --mask-length 30 --vpn-tunnel vpc-demo-tunnel1 --region us-east1
gcloud compute routers add-bgp-peer vpc-demo-router1 --peer-name bgp-on-prem-tunnel1 \
  --interface if-tunnel1-to-on-prem --peer-ip-address 169.254.1.2 --peer-asn 65002 --region us-east1

gcloud compute routers add-interface on-prem-router1 --interface-name if-tunnel0-to-vpc-demo \
  --ip-address 169.254.0.2 --mask-length 30 --vpn-tunnel on-prem-tunnel0 --region us-east1
gcloud compute routers add-bgp-peer on-prem-router1 --peer-name bgp-vpc-demo-tunnel0 \
  --interface if-tunnel0-to-vpc-demo --peer-ip-address 169.254.0.1 --peer-asn 65001 --region us-east1
gcloud compute routers add-interface on-prem-router1 --interface-name if-tunnel1-to-vpc-demo \
  --ip-address 169.254.1.2 --mask-length 30 --vpn-tunnel on-prem-tunnel1 --region us-east1
gcloud compute routers add-bgp-peer on-prem-router1 --peer-name bgp-vpc-demo-tunnel1 \
  --interface if-tunnel1-to-vpc-demo --peer-ip-address 169.254.1.1 --peer-asn 65001 --region us-east1
